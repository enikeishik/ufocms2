<?php
define('ROOT', '../../');
require '../config-cctests.php';
require ROOT . 'ufocms/Frontend/Struct.php';
require ROOT . 'ufocms/Frontend/Config.php';
require ROOT . 'ufocms/Backend/Config.php';
require ROOT . 'ufocms/Frontend/Db.php'; //not Backend, no need Audit
require ROOT . 'ufocms/Backend/Roles.php';
...
class RolesTests extends TestCase
{
    ...
    protected function createTestObject() //to avoid Roles methods caching
    {
        if (null !== $this->testObj) {
            unset($this->testObj);
            $this->testObj = null;
        }
        $this->testObj = new Roles(self::$config, self::$db);
    }
    
    protected static function setDbData()
    {
    ...
    protected static function removeDbData()
    {
    ...
    {
        $result = $this->testObj->rolesRestricted($userId, $module, $sectionId);
    ...
    {
        $result = $this->testObj->rolesPermittedAction($userId, $module, $action);
        $this->assertEquals($expected, $result);
    }
    ...
    {
        $result = 
            !$this->testObj->rolesRestricted($userId, $module, $sectionId) 
            && 
            $this->testObj->rolesPermittedAction($userId, $module, $action);
        $this->assertEquals($expected, $result);
    }
}
