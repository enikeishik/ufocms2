...
<?php $similar = $this->getSimilarItems(3, $item['Id']); if (0 < count($similar)) { ?>
        <yandex:related>
<?php foreach ($similar as $si) { ?>
        <link url="http://<?=$_SERVER['HTTP_HOST']?><?=$section['path']?><?=$si['Id']?>"><?=htmlspecialchars($si['Title'], ENT_QUOTES)?></link>
<?php } ?>
        </yandex:related>
<?php } ?>
    </item>
